...
namespace {
    ...
    class MPAR_UpdateCallback : public QCAR::UpdateCallback {
        virtual void QCAR_onUpdate(QCAR::State& state);
    } qcarUpdate;
    
    // NSerror domain for errors coming from the Sample application template classes
    NSString * MPAR_ERROR_DOMAIN = @"mpar_application";
}

@interface MPARSession ()
...
@implementation MPARSession
@synthesize viewport;

- (id)initWithDelegate:(id<MPARControl>) delegate
{
    self = [super init];
    if (self) {
...
- (NSError *) NSErrorWithCode:(int) code {
    return [NSError errorWithDomain:MPAR_ERROR_DOMAIN code:code userInfo:nil];
}
...
- (void)initQCARInBackground
{
    ...
    @autoreleasepool {
        QCAR::setInitParameters(mQCARInitFlags, "__replace__with__your__key__");
...
- (void) errorMessage:(NSString *) message {
    UIAlertView *alert = [[UIAlertView alloc] initWithTitle:MPAR_ERROR_DOMAIN
                                                    message:message
                                                   delegate:nil
...
void MPAR_UpdateCallback::QCAR_onUpdate(QCAR::State& state)
{
    if (instance != nil) {
